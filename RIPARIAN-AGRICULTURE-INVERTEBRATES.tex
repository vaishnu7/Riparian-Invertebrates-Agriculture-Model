...
%Unfortunately, in agricultural catchments, the riparian zones are frequently severely damaged. At various geographical scales, land use change, deforestation of trees or vegetation, overgrazing, pesticide inputs, and nutrients from agricultural sources all pose a danger to riparian biodiversity and ecosystem services \cite{burdon2013habitat}. There are several studies conducted at various catchment level to determine various affects of intense human disturbances on riparian vegetation (\cite{cesarini2022riparian};\cite{alemu2018identifying};\cite{heartsill2003riparian};\cite{corbacho2003patterns};\cite{schlosser1981riparian}). As a result, riparian zone protection and enhancement are frequently considered as the initial steps toward correcting the effects of agricultural productivity.\\
%Despite the fact that riparian buffers are recognised as important elements for preserving ecological balance, there are still voids in our understanding.
...
%include invertebrates, have investigated the impact of buffer features \cite{forio2020small};\cite{flory1999};\cite{kawaguchi2001}. However, models addressing the interactions of terrestrial invertebrate communities with that of riparian vegetation are relatively rare \citep{steward2022, wipfli1997, sabo2002}. Some research papers are also concerned with the effects of anthropogenic disturbances, like as agriculture, mining, etc. on riparian ecosystems \citep{you2015, burdon2020}. Aside from that, case studies on the interconnections between terrestrial invertebrates, riparian vegetation, and agricultural productivity are also conducted \citep{popescu2021riparian, delong1998, connolly2016, moore2005}.
...
%The model study demonstrates the circumstances under which agricultural production, riparian vegetation, and terrestrial invertebrates can coexist to create a stable system. These intuitive conclusions are supported by quantitative data in which the sensitivity of different model parameters to model outcomes is studied using the differential sensitivity analysis approach.
%Due to a lack of sufficient temporal data, we developed a dimensionless model that may be fitted in any data set when such investigations are undertaken by ecologists in the future.\\
%To address interactive dynamics that exist among several components, nonlinear mathematical models in terms of system of differential equations have long been recognized as efficient mathematical tools.     %To address such voids, nonlinear mathematical models in terms of system of differential equations
%In this context, our nonlinear modelling focuses on how wooded riparian buffers have temporal effects on riparian invertebrate groups under agricultural disturbances. Due to a lack of sufficient temporal data, we developed a dimensionless model that may be fitted in any dataset when such investigations are undertaken by ecologists in the future.\\
\vspace{-1cm}
...
%We reasoned that invertebrates interact with riparian vegetation as they require forest resources to survive which should be considered as a negative influence on growth of riparian vegetation.
...
%demonstrate the direct influence of intre-specific competition on carrying capacity of invertebrates.
...
%agricultural productivity, on the other hand, will have a positive impact since some invertebrate species have been shown to be beneficial to crop development as natural pest killers \citep{krell2015aquatic, riis2020global, stockan2014effects, cole2012riparian} or temperature sensors \cite{greenwood1995patial} which is denoted by the coefficient $\lambda$.
%As a result, we have demonstrated this detrimental influence on vegetation growth at a rate of $\alpha$. 
\end{enumerate}
...
%To obtain feasible $\tilde V$, different cases with respect to discriminant $\Delta_{E_3}=\tilde b^{2}-4\tilde a \tilde c$ of equation \eqref{sec3:e18} are discussed in Table \ref{Table 2}. Considering different cases (Table \ref{Table 2}), it can easily be established that there exists only one positive $\tilde V_1$ for all values of $\tilde b$, so\\
...
%&=\tilde V \tilde I \tilde A (r\gamma + r\delta + \alpha \gamma \tilde A + \alpha \delta \tilde A + \theta \beta^2 + \alpha \tilde V \theta \beta)
%\end{align}
...
\end{enumerate}
 \end{theorem}
%\item Riparian vegetation and terrestrial invertebrates - free equilibrium $E_1(0,0,1)$ is always unstable.
%\item Agricultural production - free equilibrium $E_2(\hat V,\hat I,0)$ is always  unstable.
%\item The feasible coexistence equilibrium $E_3(\tilde V,\tilde I,\tilde A)$ exhibits local asymptotic stable behavior if $\beta\geq 1$. 
...
%\mathbb I &=(V-\tilde V)\frac{1}{V}\frac{dV}{dt}\\
               %&=-(r+\alpha A)(V-\tilde V)^2-\alpha V^*(A-\tilde A)(V-\tilde V)-\beta(I-\tilde I)(V-\tilde V)
%end{align}
%\begin{align}\label{sec3:e39}
%\mathbb {II} &=(I-I^*)\frac{1}{I}\frac{dI}{dt}\\
                   %&=\theta\beta(V-\tilde V)(I-\tilde I)-\gamma(I-\tilde I)^2-\delta(A-\tilde A)(I-\tilde I)
%\end{align}
%\begin{align}\label{sec3:e40}
%\mathbb {III} &=(A-\tilde A)\frac{1}{A}\frac{dA}{dt}\\
                    %&=-(A-\tilde A)^2+\delta(I-\tilde I)(A-\tilde A)
%\end{align}
...
\end{equation}
%With these numerical parameters, it has been found that the system exhibits local as well as global asymptotic stable behavior around coexistence equilibrium $E_3$, satisfying its related mathematical conditions. 
...
%To demonstrate the behaviour of the model, we investigated each parametric value mentioned in Table \ref{Table 1}, while maintaining the others constant. We assumed that the starting values of $V$, $I$, and $A$ as $0.2, 0.3$, and $0.5$, respectively. The values investigated here all meet the Lyapunov stability requirement, and the remaining observations are as follows:
\begin{itemize}
%\item[$\bullet$] We experimented with three alternative values of the parameter $r$. We observed periodic activity in the graph at this point, and convergence occurs when we extend the time duration. However, at smaller values such as $0.009$, we can observe that the convergence is achieved without much periodicity, as seen in \cref{fig:time_vs_riparian_r}, \cref{fig:time_vs_invertebrates_r}, \cref{fig:time_vs_agriculture_r} and \cref{fig:rip_inv_agr_r}. Even though there is periodicity in the graph for larger values of $r$, such as $1.9$, the convergence occurs at a comparable rate to that of lesser values of $r$. The majority of complex eigenvalues are negative which exhibits stability.\\
