...
%\item In the cases of feasible coexistence equilibrium with $\tilde a<0$, both the conditions $b_3>0$, $b_1b_2-b_3>0$ may not hold true. But, in the cases, when $\tilde a<0$ together with $\tilde b>0$, $\tilde c>0$ or with $\tilde b<0$, $\tilde c=0$, then $b_3$ is always less than zero, so, coexistence equilibrium is unstable. However, in other cases of feasible coexistence equilibrium, with $\tilde a<0$, coexistence equilibrium is locally asymptotically stable provided $b_3>0$ and $b_1b_2-b_3>0$.
%\item In the cases of feasible coincident equilibrium with $\Delta_{E_3}=0$, $b_3$ reduces to zero due to the term $\tilde a\tilde W(2+\gamma \tilde W)-\gamma \tilde c-\tilde b$ in equation \eqref{sec3:e34}. Hence, one of the eigenvalue of the characteristic equation becomes zero and the coexistence equilibrium becomes saddle node.
